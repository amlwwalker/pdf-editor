...
type FileModel struct {
	...
	_ func(row int, filePath, fileName, fileSize, fileSource string) `slot:"editFile"`
...
func (m *FileModel) editFile(row int, filePath string, fileName string, fileSize string, fileSource string) {
	var p = m.Files()[row]
	...
	m.DataChanged(fIndex, fIndex, []int{FilePath, FileName, FileSize, FileSource})
}
